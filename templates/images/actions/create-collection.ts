...
export default defineAction({
  description:
    "Create a category-specific collection inside an image library, such as blog heroes, diagrams, product shots, or landing page images.",
  schema: z.object({
    libraryId: z.string(),
  ...
  run: async (args) => {
    ...
    await getDb().insert(schema.imageCollections).values(row);
    return row;
  },
});
