...
import { IMAGE_MODELS } from "../shared/api.js";
...
export default defineAction({
  ...
  run: async (args) => {
    ...
    const filters = [];
    if (scope.orgId)
      filters.push(eq(schema.imageGenerationRuns.orgId, scope.orgId));
    if (scope.ownerEmail) {
      filters.push(eq(schema.imageGenerationRuns.ownerEmail, scope.ownerEmail));
    }
    if (args.dateFrom) {
      filters.push(gte(schema.imageGenerationRuns.createdAt, args.dateFrom));
    }
    if (args.dateTo) {
      filters.push(lte(schema.imageGenerationRuns.createdAt, args.dateTo));
    }
    if (args.cursor) {
      // Cursor is the previous page's last `createdAt`. We're sorted DESC, so
      // "next page" means strictly older.
      filters.push(
        sql`${schema.imageGenerationRuns.createdAt} < ${args.cursor}`,
      );
    }
    if (args.ownerEmail) {
      filters.push(eq(schema.imageGenerationRuns.ownerEmail, args.ownerEmail));
    }
    if (args.libraryId) {
      filters.push(eq(schema.imageGenerationRuns.libraryId, args.libraryId));
    }
    if (args.model) {
      filters.push(eq(schema.imageGenerationRuns.model, args.model));
    }
    if (args.status) {
      filters.push(eq(schema.imageGenerationRuns.status, args.status));
    }
    if (args.source) {
      filters.push(eq(schema.imageGenerationRuns.source, args.source));
    }
    if (args.callerAppId) {
      filters.push(
        eq(schema.imageGenerationRuns.callerAppId, args.callerAppId),
      );
    }
    if (args.promptSearch) {
      filters.push(
        like(schema.imageGenerationRuns.prompt, `%${args.promptSearch}%`),
      );
    }
    ...
    const libs =
      libIds.length === 0
        ...
        : await db
            .select({
              id: schema.imageLibraries.id,
              title: schema.imageLibraries.title,
              ownerEmail: schema.imageLibraries.ownerEmail,
            })
            ...
            .where(
              libIds.length === 1
                ? eq(schema.imageLibraries.id, libIds[0])
                : sql`${schema.imageLibraries.id} IN (${sql.join(
    ...
    let childCounts = new Map<string, { total: number; saved: number }>();
    if (page.length) {
      const runIds = page.map((r) => r.id);
      const childRows = await db
        .select({
          runId: schema.imageAssets.generationRunId,
          status: schema.imageAssets.status,
        })
        ...
        .where(
          runIds.length === 1
            ? eq(schema.imageAssets.generationRunId, runIds[0])
            : sql`${schema.imageAssets.generationRunId} IN (${sql.join(
