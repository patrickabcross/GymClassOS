...
function AddIntegrationButton() {
  ...
  const handleSubmit = () => {
    ...
    send({
      ...
      context: `The user wants to add a new integration for "${name}" to the contact sidebar. Look at the existing integrations in app/components/calendar/IntegrationsSidebar.tsx and the pattern in server/routes/ and app/hooks/use-integrations.ts. Add the new provider following the same pattern: server route, hook, sidebar section, and logo. Use the real brand logo SVG if possible.`,
      submit: true,
      requiresCode: true,
    });
    setValue("");
