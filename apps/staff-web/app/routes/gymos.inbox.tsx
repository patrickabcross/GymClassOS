...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "send-template") {
    ...
    return redirect(`/gymos/inbox?conversation=${conversationId}&sent=1`);
  }
    ...
    <div className="flex h-full w-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {data.conversations.map((c) => {
            ...
            return (
              <Link
                key={c.id}
                to={`/gymos/inbox?conversation=${c.id}${data.isLeadsView ? "&filter=leads" : ""}`}
                preventScrollReset
                className={cn(
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        {!data.selectedConversation ? (
          ...
          // placeholder. Coaches landing on /gymos/inbox with no thread selected
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center">
            ...
            {data.conversations.length > 0 ? (
              <Link
                to={`/gymos/inbox?conversation=${data.conversations[0].id}`}
                preventScrollReset
