...
- All video actions are DIRECT (no approval gate needed) — staff-only authoring. spec is a structured JSON object (VideoSpec: {format:"square"|"landscape", fps, durationInFrames, scenes:[{type:"title"|"textOverImage"|"outro", text, subtitle?, imageUrl?, bgColor?, durationInFrames}]}). Compositions stay 'draft' throughout (member-facing publishing arrives in CV4). Use list-classes to ground class details before prefilling a promo spec. To edit scene copy, always call video-get-composition first to read the current spec, then call video-update-composition with the COMPLETE modified spec.
