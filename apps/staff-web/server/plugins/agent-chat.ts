...
- deactivate-schedule-rule — stop a recurring rule so the nightly materialiser no longer generates future occurrences ({id}). Already-generated occurrences are preserved with their own lifecycle. Returns {deactivated:true} | {error:"RULE_NOT_FOUND"|"RULE_ALREADY_INACTIVE"}.
