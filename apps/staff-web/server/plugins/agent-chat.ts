...
- suggest-template-vars — fill in a WhatsApp template's {{N}} variables for the open inbox conversation, then write them back for the coach to review. Treat each {{N}} as a blank inside a sentence, not a keyword slot: read the words immediately before and after the placeholder in the template body and choose wording so the FINISHED message reads as one natural, warm, conversational sentence — correct grammar, article, tense and number, lowercase unless it's a proper noun. Write what a friendly human coach would actually say, NOT a raw data label, product name, or category (e.g. for "…we know {{2}} can feel daunting…" prefer "trying a new class" over "Drop-in Class"). {{1}} is usually the member's first name. Pass conversationId, templateName, and a vars map (e.g. {"1":"Sarah","2":"trying a new class"}). This does NOT send the message — the coach reviews and sends.
...
- create-schedule-rule — create a RECURRING class rule that repeats on chosen days of the week ({definitionId, daysOfWeek: number[], timeOfDay: "HH:MM", startsOn: "YYYY-MM-DD", location?, capacity?, trainerId?, endsOn?}). daysOfWeek uses weekday numbers (0=Sun, 1=Mon, … 6=Sat). timeOfDay is Europe/London studio-local time (BST/GMT auto-corrected). Immediately generates the first 8 weeks of occurrences. Returns {id, definitionId, daysOfWeek, timeOfDay, startsOn, occurrencesGenerated}.
- update-schedule-rule — patch mutable fields on a schedule rule ({id, timeOfDay?, location?, capacity?, trainerId?, endsOn?}). Identity fields (definitionId, daysOfWeek, startsOn) cannot be changed — deactivate and recreate instead. Returns {updated:true} | {updated:false, reason} | {error:"RULE_NOT_FOUND"}.
- deactivate-schedule-rule — cancel a recurring series ({id}): stops the nightly materialiser AND cancels future not-yet-started occurrences that have no bookings. Future occurrences with a booked member are kept (cancel those individually via propose-action cancel-occurrence so pass credits refund). Returns {deactivated:true, occurrencesCancelled, occurrencesKeptWithBookings} | {error:"RULE_NOT_FOUND"|"RULE_ALREADY_INACTIVE"}.
...
- dispatch-video-brief — send a saved brief to the coach's WhatsApp as an approved template so they can go shoot it ({briefId}, or omit to send the most recent draft). Use when the coach says "send me that brief" / "WhatsApp me the shoot". Delivery needs operator setup (an approved 'video_brief' template, the coach as an opted-in member, and the COACH_WHATSAPP_E164 env); if anything is missing it returns a clear {error} (e.g. TEMPLATE_NOT_APPROVED, COACH_NOT_CONFIGURED, COACH_NOT_A_MEMBER) — relay that to the coach so they can finish setup. Returns {dispatched:true, briefId, template}.
