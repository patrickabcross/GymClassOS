 ...
 *
 * Forked from: templates/content/actions/list-documents.ts
 ...
 *   - No functional changes required; accessFilter + ownableColumns org-scope
 *     naturally scopes to HQ_ORG_ID for the super-admin.
 */
...
export default defineAction({
  description:
    "List HQ content document metadata ordered by position. Does not return full document bodies; use content-get-document for one document's content.",
  schema: z.object({}),
  http: { method: "GET" },
  run: async () => {
    const db = getDb();
