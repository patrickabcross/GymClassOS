...
Last activity: 2026-07-01 — Completed quick task 260701-dyk: EXPO_PUBLIC_API_BASE on the preview-install EAS profile (standalone iOS build targets live backend). Also this session (not quick tasks): activated mobile admin (owner AI) + teacher surfaces on prod for UAT — set RUNSTUDIO_OPERATOR_EMAILS + RUNSTUDIO_TEACHER_EMAILS (deploy fac67ba1), created owner.test/teacher.test accounts, linked trainers.trn_seed_12 → teacher, seeded a booked member into a teacher class; all role gating API-verified.
